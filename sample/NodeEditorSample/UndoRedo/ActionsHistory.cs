...
public class ActionsHistory : IActionsHistory
{
    ...
    public static readonly ActionsHistory Global = new();
    private readonly List<IAction> _batchHistory = new();
    private readonly List<IAction> _history = new();
    ...
    public void Record(IAction op)
    {
        ...
        if (_isApplyingOperation || !IsEnabled) return;
    ...
    public void Undo()
    {
        if (IsBatching) throw new InvalidOperationException($"{nameof(Undo)} is not allowed during a batch.");
    ...
    public void Redo()
    {
        if (IsBatching) throw new InvalidOperationException($"{nameof(Redo)} is not allowed during a batch.");
